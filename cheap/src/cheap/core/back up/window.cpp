...
namespace cheap {
	...
	window::window_data::window_data(const unsigned                     aWindow_width, const unsigned aWindow_height,
		...
		: width(aWindow_width), height(aWindow_height), event_callback(aEvent_callback_function)
	{
	}
	...
	window::window(
		...
		mStart_height_(window_height),
		m_aspect_ration_(16.0f / 9.0f),
		m_x_(x),
		m_y_(y),
		m_window_data_(window_width, window_height, aEvent_callback_function),
		m_is_fullscreen_(is_fullscreen),
		m_icons_(/*new GLFWimage(0, 0, nullptr)*/),
		m_update_callback_(aUpdate_callback)
	{
		LOG();
		if (m_init())
	...
	float window::get_aspect_ration() const { return m_aspect_ration_; }

	int window::get_start_width() const { return m_start_width_; }

	int window::get_start_height() const { return mStart_height_; }
	...
	void window::set_fullscreen(const bool aIs_fullscreen)
	{
		...
		if (aIs_fullscreen) {
			...
			glfwSetWindowMonitor(CP_GLFW_WINDOW_POINTER, primary_monitor, 0, 0, video_mode->width, video_mode->height,
				video_mode->refreshRate);
		} else {
			if (!m_is_fullscreen_) return;
			glfwSetWindowMonitor(CP_GLFW_WINDOW_POINTER, nullptr, m_x_, m_y_, m_start_width_, mStart_height_,
				GLFW_DONT_CARE);
		}

		m_is_fullscreen_ = aIs_fullscreen;
	}
	...
	void window::set_icon(const std::string& image_path) const
	{
		...
		CP_STB_IMAGE_ICON_POINTER->pixels = stbi_load(image_path.c_str(), &CP_STB_IMAGE_ICON_POINTER->width,
			&CP_STB_IMAGE_ICON_POINTER->height, nullptr, 4);
	...
	void window::set_up_window(const bool turn_on_vsync)
	{
		...
		glfwSetWindowUserPointer(CP_GLFW_WINDOW_POINTER, &m_window_data_);
		...
		glfwSetFramebufferSizeCallback(
			...
			[](GLFWwindow* window, const int width, const int height) {
				...
				const auto event = new
					app_window_resize_event(static_cast<unsigned int>(width),
