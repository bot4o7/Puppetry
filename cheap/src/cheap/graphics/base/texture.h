...
namespace cheap {
	...
	// Constructor for rectangles
		texture()
			: m_image_path_(""), m_tex_slot_(0), m_width_(1),
		...
		explicit texture(const std::string& image_path)
			: m_image_path_(image_path), m_tex_slot_(1), m_width_(1),
			m_height_(1), m_bytes_per_pixel_(4), m_color_format_(color_format::rgba), m_image_data_(nullptr)
		{
			// Allocate space and bind a 2D texture
			glGenTextures(1, &m_id_);
		...
		explicit texture(const font& font)
			: m_image_path_(""), m_tex_slot_(1), m_width_(font.get_atlas_width()), m_height_(font.get_atlas_height()),
		...
		[[nodiscard]] unsigned int get_tex_slot() const { return m_tex_slot_; }

		[[nodiscard]] int get_width() const { return m_width_; }

		[[nodiscard]] int get_height() const { return m_height_; }

		void set_color_format(const color_format format) { m_color_format_ = format; }
